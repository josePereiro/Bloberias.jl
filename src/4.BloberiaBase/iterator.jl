...
function eachbatch(B::Bloberia, bbid_prefix = nothing; 
        sortfun = identity, 
        ch_size::Int = get(getmeta(B), "eachbatch.ch_size", nthreads()),
        n_tasks::Int = get(getmeta(B), "eachbatch.n_tasks", nthreads()),
        preload = get(getmeta(B), "eachbatch.preload", String[])
    )
    if n_tasks > 1
        return _eachbatch_th(B, bbid_prefix; sortfun, ch_size, n_tasks, preload)
    else
        return _eachbatch_ser(B, bbid_prefix; sortfun, ch_size, preload)
    end
end
...
function foreach_batch(f::Function, B::Bloberia, bbid_prefix = nothing; 
        ...
        lk = get(getmeta(B), "foreach_batch.preload", false),
    )
    if n_tasks > 1
