...
function _is_valid_access(::BlobBatch, fT) 
    ...
    fT == bbFRAME_FRAME_TYPE && return true
...
_getbframe!(bb::BlobBatch, id)=
    _getframe!(bb, id, bFRAME_FRAME_TYPE, VFRAME_DEPOT_TYPE)
...
_getbbframe!(bb::BlobBatch, id) =
    _getframe!(bb, id, bbFRAME_FRAME_TYPE, DFRAME_DEPOT_TYPE)
...
function getframe!(bb::BlobBatch, id, fT::Symbol)
    _check_access(bb, fT)
    fT == bFRAME_FRAME_TYPE && return getbframe!(bb, id)
    fT == bUUIDS_FRAME_TYPE && return getbuuids(bb)
    fT == bbFRAME_FRAME_TYPE && return getbbframe!(bb, id)
    return getmeta(bb)
end
