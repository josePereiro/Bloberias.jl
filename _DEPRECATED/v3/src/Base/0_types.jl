...
# lite (file: lite.frame.jls) [lite/blob_uuid/group/key]
#   |-- 0x1a
#        |-- {"0" => {"alpha" => 1, ...}, ...}
#   |-- 0xf1
#        |-- {"0" => {"alpha" => 2, ...}, ...}

#  nonlite [nonlite/group/blob_uuid/key]
...
mutable struct BlobBatch
    ...
    # disk
    meta::OrderedDict      # config/state/meta in general
    uuids::Vector{UInt128} # defines which blobs are present
    lite::OrderedDict      # lite frames
    nonlite::OrderedDict   # non-lite frames
    # ram
    temp::OrderedDict      # RAM only
end
